...
// inspectAWSLoadBalancerForPGP performs AWS load balancer crypto inspection for PGP command
func inspectAWSLoadBalancerForPGP() map[string]string {
	...
	discoverAWSLoadBalancersForPGP(instanceID, results)
	
	fmt.Println("\nAWS LOAD BALANCER CONTEXT:")
	fmt.Printf("PGP key management running on AWS environment (EC2 Instance: %s)\n", instanceID)
	fmt.Println("Note: PGP analysis shows local key management configuration.")
	fmt.Println("If PGP keys are used by web applications, internet-facing traffic may use different crypto via load balancer.")
	
	return results
}

// discoverAWSLoadBalancersForPGP discovers load balancers for PGP-related services
func discoverAWSLoadBalancersForPGP(instanceID string, results map[string]string) {
			...
			for _, lb := range elbResponse.LoadBalancerDescriptions {
				for _, instance := range lb.Instances {
					if instance.InstanceId == instanceID {
						...
						// Analyze web-related listeners that might use PGP-signed content
						analyzePGPLoadBalancerListeners(lb.ListenerDescriptions, results)
						return
					}
				}
			}
						...
						for _, target := range healthResponse.TargetHealthDescriptions {
							if target.Target.Id == instanceID {
								for _, lbArn := range tg.LoadBalancerArns {
									analyzePGPApplicationLoadBalancer(lbArn, results)
									return
								}
							}
						}
...
// analyzePGPLoadBalancerListeners analyzes listeners for services that might use PGP
func analyzePGPLoadBalancerListeners(listeners []struct {
	Listener struct {
		Protocol         string `json:"Protocol"`
		LoadBalancerPort int    `json:"LoadBalancerPort"`
		SSLCertificateId string `json:"SSLCertificateId"`
	...
	for _, listener := range listeners {
		...
		// Check for web-related ports that might serve PGP-signed content (HTTP: 80, HTTPS: 443)
		if port == 80 || port == 443 || port == 8080 || port == 8443 {
			webPorts = append(webPorts, fmt.Sprintf("%d/%s", port, protocol))
	...
	if len(webPorts) > 0 {
		results["LB Web Ports"] = strings.Join(webPorts, ", ")
		results["LB Primary Port"] = webPorts[0]
		
		if len(sslPolicies) > 0 {
			results["LB SSL Policy"] = sslPolicies[0]
			analyzeSSLPolicyForPGP(sslPolicies[0], results)
		} else {
			results["LB SSL Context"] = "No SSL termination"
			results["LB PQC Ready"] = "N/A"
		}
	} else {
		results["LB Web Ports"] = "No web ports found"
		results["LB SSL Context"] = "No web services"
		results["LB PQC Ready"] = "N/A"
	}
}

// analyzePGPApplicationLoadBalancer analyzes ALB/NLB for services that might use PGP
func analyzePGPApplicationLoadBalancer(lbArn string, results map[string]string) {
	...
	for _, listener := range listenersResponse.Listeners {
		// Check for web-related ports that might serve PGP-signed content
		if listener.Port == 80 || listener.Port == 443 || listener.Port == 8080 || listener.Port == 8443 {
			webPorts = append(webPorts, fmt.Sprintf("%d/%s", listener.Port, listener.Protocol))
	...
	if len(webPorts) > 0 {
		...
		if sslPolicy != "" {
			...
			analyzeSSLPolicyForPGP(sslPolicy, results)
		} else {
			results["LB SSL Context"] = "No SSL termination"
			results["LB PQC Ready"] = "N/A"
		}
...
// analyzeSSLPolicyForPGP analyzes SSL policy for services that might use PGP
func analyzeSSLPolicyForPGP(sslPolicy string, results map[string]string) {
	if sslPolicy == "" {
		return
	}
